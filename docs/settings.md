...
something permanent, or put it in a profile to affect one account.
...
refused. An unsafe file aborts the command with a clear message; these files
are trusted input and must be owned by you and not group- or other-writable.

One compatibility layer sits between the shipped defaults and your local
files: the `OWNCLOUD_*` aliases of `nextcloudcmd`/the desktop client are
applied after `config/settings.env` and before `config/settings.local.env`
(see [nextcloudcmd and desktop client aliases](#nextcloudcmd-and-desktop-client-aliases)).
...
A profile is an independent account. Selecting one with `--profile NAME` (or
`SCIEBO_PROFILE=NAME`) moves the derived defaults into the profile:
...
Profile names may contain letters, digits, dot, dash, and underscore; `default`
is reserved for the project-wide layout. Manage profiles with
`sciebo account` (see [docs/commands.md](commands.md#account)).
...
configuration base: the settings files, manifests, filters, and profiles are
read from `DIR`, and the default state directory becomes `DIR/state`. Only
the settings file falls back to the shipped `config/settings.env` when `DIR`
has none; `.env` stays in the project root. `--log-dir DIR` overrides
`LOG_DIR` for the run, and `--log-expire HOURS` sets the runtime
`LOG_EXPIRE_HOURS` that `cleanup --logs` honors. All three may appear
anywhere on the command line (they are consumed before the command parser).

### sources.conf and folders.conf

All manifest files share one format:

```
mode | local_path | remote_subdir [ | filter_file ]
```

- `mode`: `sync` (upload), `pull` (download), or `bisync` (two-way).
...
`config/sources.generated.conf`. Git only locates folders; contents, including
`.git/`, are synced like any other files.
...
`nextcloudcmd` and the Nextcloud desktop client are configured through
`OWNCLOUD_*` environment variables. For compatibility, these are mapped onto
the sciebo settings after `config/settings.env` is sourced and before
`config/settings.local.env`; an exported sciebo setting wins over its alias,
and a plain assignment in a local or profile file beats both. Empty or
unrecognized alias values are ignored.
...
| --- | --- | --- |
| `RCLONE_REMOTE` | `sciebo` | name of the rclone remote (and keychain account). |
...
| `BISYNC_RESYNC_MODE` | `newer` | `--resync-mode` on `--resync` runs; same values as `BISYNC_CONFLICT_RESOLVE`. |
...
| `SKIP_HIDDEN` | `0` | `0` syncs dot-files/directories (desktop-client behavior); `1` excludes them (`nextcloudcmd` default). |
...
Folder and name guards modeled on the Nextcloud desktop client. The wizard
applies them when adding a pair, sync preflight applies them per entry, and
`doctor` scans and reports the active policies. A condition that the policy
does not allow skips the entry or pair with a warning instead of failing the
run.
...
| `E2EE_POLICY` | `exclude` | server-side end-to-end encrypted folders on pull/bisync: `exclude` excludes them, `warn` only reports, `allow` syncs the encrypted blobs. An entry whose remote root is E2EE is skipped unless `allow`. |
...
The E2EE and external-storage checks need a Nextcloud WebDAV remote and a
server that exposes the property; on other setups they stay silent.

### Delete guard and local trash

| Setting | Default | Meaning |
| --- | --- | --- |
| `DELETE_FILES_THRESHOLD` | `100` | deletion count at which the guard stops a run (non-negative integer). |
...
| `BLACKLIST_MAX_FAILS` | `3` | a path erroring this many times is excluded from later runs until `sciebo retry` clears it. |
...
[Precedence](#precedence)); `TLS_INSECURE`/`--trust` is independent.
...
| `CLIENT_KEY_PASSWORD` | empty | plaintext passphrase for an encrypted `CLIENT_KEY`; rclone receives an obscured `--client-pass` (sciebo obscures it first, because rclone expects an obscured value), and curl receives it through a mode-600 `--config` file (`pass = "..."`), never `--pass` in the argv. |
...
| Setting | Default | Meaning |
| --- | --- | --- |
| `KEYCHAIN` | `1` | `1` stores the plaintext app password in the platform keychain (`security`, `secret-tool`, or `pass`) so HTTP commands never reveal it; `0` keeps the obscured password in the rclone config. |
| `KEYCHAIN_SERVICE` | `rclone-sciebo` | keychain service name; a named profile gets `rclone-sciebo/<name>` automatically when this is unchanged. The plaintext item uses account `<RCLONE_REMOTE>#plain`; a legacy obscured item under account `RCLONE_REMOTE` is migrated on first use. |

### Server capabilities cache

| Setting | Default | Meaning |
| --- | --- | --- |
| `CAPABILITIES_MAX_AGE` | `86400` | seconds a cached OCS capabilities probe stays fresh. |
...
Used by `sciebo schedule` with the launchd (macOS) or `systemd --user`
(Linux) backend. On a system with only `crontab`, the command refuses to
manage the schedule and tells you to edit the crontab yourself.
...
| `SCHEDULE_PROFILES` | empty | extra profiles to render one agent each for, labelled `<LAUNCHD_LABEL>.<profile>` and run with `--profile`; comma or space separated; `--profiles` overrides. Empty means only the active profile. |

### Automatic sync (watch)

`sciebo watch` watches the local directories of the configured sources and
runs `sync --apply --quiet --only NAME` when one changes.
...
| Setting | Default | Meaning |
| --- | --- | --- |
| `METERED_POLICY` | `allow` | `allow`, `ask`, or `skip` on a metered connection; `ask` prompts on a TTY and skips otherwise. `sync --metered-ok` overrides it for one run. |
...
- A legacy install that still holds the obscured password under account
  `RCLONE_REMOTE` is migrated once on first use: the obscured item is revealed
  in memory and rewritten to the `#plain` slot. The legacy item is left in
  place (the migration never reads it again) and `logout` deletes both items.
...
is still revealed with `rclone reveal` when the HTTP layer needs the plaintext.
`doctor` fails when `KEYCHAIN=1` is set but no backend exists, reports the
active backend, and warns when a keychain-enabled setup still carries the
...
verify certificates by default as well; `--trust`/`TLS_INSECURE=1` disables
that check too, printing a one-time warning.
...
support is not guaranteed), so its credentials remain argv-visible.
`PROXY_DIRECT=1` strips `HTTP_PROXY` / `HTTPS_PROXY` (and their lowercase
forms, plus `ALL_PROXY`) from rclone's child environment and passes
`--noproxy '*'` to curl. When both are set, `PROXY_DIRECT` wins (`doctor` and
`network` report the effective mode). `PROXY_TYPE` selects which of these
layers applies: `system` (the default) keeps the behavior above, `none`
ignores all proxy settings, and `http`/`socks5` take the proxy from `PROXY`
(missing `PROXY` is an error, never a silent direct connection).
...
Every path can be overridden from the environment; commands never write
outside them. This is how the test suites stay isolated.
...
With the default profile, `STATE_DIR` is `state/` in the project checkout
(under `<confdir>/state` when `--confdir` is used). Named profiles use
`state/profiles/<name>/`.
...
| `state/blacklist/<name>` | failure-count records for the blacklist |
...
| `state/capabilities.env`, `state/capabilities.json` | cached OCS capabilities (parsed and raw) |
...
| `state/nextcloudcmd/<key>/` | per-URL bisync workdirs used by `nextcloudcmd` |
...
lists the same set ready to uncomment. Frequent changes:
...
These are read from the environment by specific commands; they exist mainly
for tests and debugging and are not part of the supported surface:
