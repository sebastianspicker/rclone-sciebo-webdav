...
#     remote_secret_plain in lib/rclone.sh).
#
...
source "${LIB_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}/platform.sh"
...
# remote_secret_invalidate in lib/rclone.sh) drops it explicitly.
...
# including the remote/HTTP caches owned by lib/rclone.sh and lib/http.sh when
# they are loaded.
keychain_delete() {
  local rc=0
