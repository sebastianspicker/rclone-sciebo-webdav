...
# now_epoch/duration_seconds come from lib/base/duration.sh, always loaded before
...
# _bigfolder_label BYTES - human label for a byte count, in capabilities.sh's
# SizeSuffix style (lib/sciebo.sh always loads capabilities.sh before this
# file, so the rclone-style formatter is always available).
_bigfolder_label() {
  format_size_bytes "$1" rclone
}
