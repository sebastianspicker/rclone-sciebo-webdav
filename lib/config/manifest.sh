...
# _manifest_list_emit_entry JSON_MODE SRC - print one row for the entry the
...
# _manifest_list_emit_invalid JSON_MODE - print the INVALID row for the line
...
# manifest_list_render JSON_MODE - print one row per parsed manifest entry
# (mode, name, local path, remote path, filter), including invalid entries
# with their error (which keep going to stderr in JSON mode). JSON_MODE 1
# wraps the valid rows in {"sources":[...]}; JSON_MODE 0 prints the aligned
# table, or the "No sources configured" notice when nothing was found.
# Shared by `sciebo list` (cmd_list) and `sciebo sync --list`, so both
# print byte-identical rows from one implementation. The caller loads
# settings first.
manifest_list_render() {
  local json_mode="${1:-0}" file="" src="" line="" count=0 content=""
  if [[ "$json_mode" -eq 1 ]]; then
    output_json_begin
  ...
  while IFS= read -r file; do
    ...
    while IFS= read -r line; do
      if manifest_parse_line "$line"; then
        count=$((count + 1))
        _manifest_list_emit_entry "$json_mode" "$src"
      else
        _manifest_list_emit_invalid "$json_mode"
      fi
    done <<<"$content"
  done < <(manifest_files)
  if [[ "$json_mode" -eq 1 ]]; then
