...
#
# Sourcing this file only defines functions and constants; no path is
# derived and no settings file is read until settings_init_paths and
...
# settings_init_paths - derive the configuration base paths from the
# environment (SCIEBO_CONFDIR, --confdir's early export) and initialize the
# profile-selection state. Idempotent: a second call is a no-op, so it is
# safe to call from both the entrypoint and load_settings.
settings_init_paths() {
  [[ -z "${_SCIEBO_SETTINGS_PATHS_INIT:-}" ]] || return 0
  _SCIEBO_SETTINGS_PATHS_INIT=1
...
# shellcheck disable=SC2120  # http_load_context below calls it with no args
load_settings() {
  settings_init_paths
  _load_settings_files
  _apply_profile_layer
...
# steps in one place instead of repeating them in every command. Lives here
# (not lib/adapters/http.sh) because it calls load_settings: an adapter must
# not call into config.
http_load_context() {
  load_settings
  http_require_curl
  http_remote_info
