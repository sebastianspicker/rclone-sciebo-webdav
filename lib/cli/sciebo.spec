# lib/cli/sciebo.spec - the declarative CLI definition: this file, not
# bin/sciebo or lib/cli/main.sh, is the single source of truth for the
# command registry. scripts/gen-cli.sh reads it and writes:
#   - the shell completions completions/sciebo.bash, completions/_sciebo
#     (zsh), and completions/sciebo.fish, and
#   - lib/cli/registry.sh, the generated runtime tables (the command list,
#     the command-to-module map, the tier map, and the global-option table)
#     that lib/sciebo.sh and lib/cli/main.sh source.
# Edit this file, then run `make gen` (or `scripts/gen-cli.sh`) to
# regenerate all four; `scripts/gen-cli.sh --check` reports drift without
# writing, and also validates the spec itself (every COMMAND's module file
# exists, every lib/commands/*.sh is referenced, and usage_main's Commands/
# Extra commands sections list exactly the spec's core/extra commands, in
# spec order).
#
...
#
# GLOBAL|<names>|<placeholder>|<kind>|<action>|<description>|<env-var>|<runtime-kind>|<early>
...
#   repeatable/two-arg today) and drive the completions only. The last
#   three fields drive the generated _SCIEBO_GLOBAL_SPECS runtime table
#   (lib/cli/registry.sh, consumed by lib/cli/main.sh's
#   _sciebo_consume_globals):
#     <env-var>      the settings variable this option exports, "-" for
#                    the version pseudo-var (see <runtime-kind> below), or
#                    empty when this row has no runtime record at all
#                    (only -h/--help: help is dispatched directly by
#                    main(), never through _sciebo_consume_globals, so it
#                    is omitted from the generated table)
#     <runtime-kind> value|flag|version, empty when <env-var> is empty:
#                      value    consume the next argument, or --flag=VALUE
#                      flag     set ENV_VAR=1 and take no argument
#                      version  print the version and exit
#     <early>        1 when this option must be consumed before
#                    settings_init_paths derives the config/state layout
#                    (so it is exported by the early pass in sciebo_main
#                    and never reaches a command parser), else 0; empty
#                    when <env-var> is empty
#
# COMMAND|<name>|<tier>|<module>|<description>
#   One row per top-level command, in the order bin/sciebo's command list
#   presents them (lib/cli/registry.sh's generated SCIEBO_COMMANDS), plus
#   a trailing `help` row (handled specially by bin/sciebo, so it is not
#   itself in SCIEBO_COMMANDS). <tier> is "core" or "extra": lib/cli/
#   main.sh's usage_main lists core commands under "Commands:" and extra
#   commands under "Extra commands:", in spec order - scripts/gen-cli.sh
#   validates that grouping and order against usage_main's actual text.
#   <module> is the lib/commands/ file that defines this command's cmd_/
#   usage_ functions, without its .sh suffix (most commands are their own
#   name, e.g. `setup|core|setup`; a handful of modules answer several
#   commands, e.g. `check|core|sync`, `resume|core|pause`,
#   `umount|core|mount`, `unlock|core|lock`, `unlimited|core|limit`); "-"
#   for the `help` row, which has no module.
#
...
GLOBAL|--profile|profile|value|PROFILES|select a profile|SCIEBO_PROFILE|value|0
GLOBAL|--trust||flag|NONE|accept invalid TLS certificates|TLS_INSECURE|flag|0
GLOBAL|--non-interactive||flag|NONE|never prompt; fail or skip instead|SCIEBO_NON_INTERACTIVE|flag|0
GLOBAL|--debug||flag|NONE|verbose rclone/HTTP output|SCIEBO_DEBUG|flag|0
GLOBAL|--log-file|file|value|FILES|write the rclone log here|SCIEBO_LOG_FILE|value|0
GLOBAL|--confdir|directory|value|DIRS|configuration and state directory|SCIEBO_CONFDIR|value|1
GLOBAL|--log-dir|directory|value|DIRS|write run logs here|SCIEBO_LOG_DIR|value|1
GLOBAL|--log-expire|hours|value|NONE|delete logs older than this many hours|SCIEBO_LOG_EXPIRE_HOURS|value|1
GLOBAL|--version,-V||flag|NONE|show the version|-|version|0

# --- commands (SCIEBO_COMMANDS order, plus help) ------------------------
COMMAND|setup|core|setup|create/update the sciebo rclone remote
COMMAND|doctor|core|doctor|preflight checks (PASS/WARN/FAIL report)
COMMAND|discover|core|discover|find git repositories under roots.conf
COMMAND|list|core|list|list configured sources
COMMAND|check|core|sync|dry run of all sources
COMMAND|sync|core|sync|apply sync/pull/bisync for all sources
COMMAND|verify|core|verify|check that sources match their destinations
COMMAND|status|core|status|last run per source, plus the pause state
COMMAND|pause|core|pause|skip sync/check runs until resumed
COMMAND|resume|core|pause|clear the pause marker
COMMAND|folders|core|folders|choose/add/list/edit/remove folder pairs
COMMAND|mount|core|mount|mount the remote on demand (nfsmount)
COMMAND|umount|core|mount|unmount a recorded mount
COMMAND|mounts|core|mount|show recorded mounts
COMMAND|cleanup|core|cleanup|old logs / stale chunk-upload housekeeping
COMMAND|schedule|core|schedule|install/uninstall/status of the launchd agent
COMMAND|trash|core|trash|list/restore/remove/empty the Nextcloud trashbin
COMMAND|versions|core|versions|list/download/restore/delete file versions
COMMAND|share|core|share|create, list, update, or remove Nextcloud shares
COMMAND|notifications|extra|notifications|list or delete Nextcloud notifications
COMMAND|activity|extra|activity|show the Nextcloud activity stream
COMMAND|presence|extra|presence|show or set your Nextcloud user status
COMMAND|lock|core|lock|lock a remote file (WebDAV)
COMMAND|unlock|core|lock|unlock a remote file (WebDAV)
COMMAND|locks|core|lock|list locks recorded by sciebo lock
COMMAND|quota|core|quota|show server quota usage
COMMAND|open|core|open|open a local sync folder
COMMAND|conflicts|core|conflicts|find local conflict copies
COMMAND|retry|core|retry|clear failure-blacklist entries
COMMAND|account|core|account|manage profiles (list/add/remove/use/info/avatar)
COMMAND|logout|core|logout|remove credentials for the active remote
COMMAND|config|core|config|show effective settings and their source
COMMAND|support|core|support|build a redacted debug archive
COMMAND|nextcloudcmd|core|nextcloudcmd|nextcloudcmd-compatible single sync run
COMMAND|watch|core|watch|run sync when local files change
COMMAND|limit|core|limit|cap bandwidth for later runs
COMMAND|unlimited|core|limit|lift the bandwidth cap
COMMAND|network|core|network|show the active network and metered state
COMMAND|filters|core|filters|manage filter files (sync/list/show/check)
COMMAND|file|extra|file|file details, activity, and shares
COMMAND|search|extra|search|unified search on the server
COMMAND|recent|extra|recent|recently modified remote files
COMMAND|comments|extra|comments|file comments (Nextcloud comments app)
COMMAND|favorites|extra|favorites|list or toggle server-side favorites
COMMAND|tags|extra|tags|list system tags and assign them
COMMAND|server|extra|server|server information and capabilities
COMMAND|hydrate|core|hydrate|download a remote path on demand
COMMAND|provision|core|provision|non-interactive account and folder setup
COMMAND|logs|core|logs|list, show, or tail per-source sync logs
COMMAND|edit|extra|edit|download, edit, and upload one remote file
COMMAND|ignored|core|ignored|list files excluded from syncing
COMMAND|announcements|extra|announcements|list server announcements
COMMAND|preview|extra|preview|download a preview image for a remote file
COMMAND|download|core|download|download a remote file or directory
COMMAND|update|core|update|update the local checkout from git
