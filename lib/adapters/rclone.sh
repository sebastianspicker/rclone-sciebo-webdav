...
# classification itself is the shared _proxy_classify (lib/adapters/proxy.sh); an
...
# in either process-lifetime cache. http.sh's HTTP_SECRET_CACHE (derived
# from the plaintext) and keychain.sh's memoized backend probe are dropped
# with them, so the next rclone_cmd re-probes after a credential change.
...
remote_configured() {
  ...
  # Capture, then match: piping into `grep -q` lets grep exit at the first
  # match while rclone is still writing later remotes; the SIGPIPE status
  # then fails the pipeline under pipefail and a configured remote reads as
  # missing (intermittently, depending on scheduling).
  local remotes="" line=""
  remotes=${ rclone_cmd listremotes 2>/dev/null;} || return 1
  while IFS= read -r line; do
    if [[ "$line" == "${RCLONE_REMOTE}:" ]]; then
      REMOTE_CONFIGURED_CACHE="1"
      return 0
    fi
  done <<<"$remotes"
...
# http_remote_info (lib/adapters/http.sh) and capabilities_base_url both delegate
...
# caches, like the config and size caches.
...
rclone_stat_is_dir() {
  ...
  if [[ "$out" =~ \"IsDir\"[[:space:]]*:[[:space:]]*true ]]; then
