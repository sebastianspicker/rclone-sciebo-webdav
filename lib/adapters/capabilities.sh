...
#
# The request goes through the shared lib/adapters/http.sh plumbing when it is
...
# safe_source_file lives in lib/base/core.sh so settings loading can use it too.
...
# missing, the transport fails, or the status is not 2xx. Uses http_curl
# (lib/adapters/http.sh, always loaded), so TLS_INSECURE, PROXY/PROXY_DIRECT,
# HTTP_TIMEOUT and the netrc password path apply.
capabilities_fetch_raw() {
  local base="$1" url="" show="" user="" secret="" body_file="" rc=0
