...
#
# `folders add` appends one pair; `folders choose` browses the remote and
# gathers pairs into the P_* state (choose_* below); `folders import`
# migrates a nextcloudcmd --unsyncedfolders list; all commit through
...
# `folders choose`'s own wizard state (cmd_choose below): gathers pairs into
# the P_* arrays above, then commits them through folders_commit_pending.
...
# folder, resolved in the caller's shell (not a command substitution).
...
declare -gA FOLDERS_LIST_LOG_INDEX=()
...
declare -gA FOLDERS_BISYNC_INIT=()
...
with `sciebo sync --resync --apply`). Run `sciebo check` for a full dry run
before `sciebo sync`.
EOF
}
...
folders_commit_pending() {
  ...
  printf 'Next: %s check (dry run), then %s sync\n' "$CLI_NAME" "$CLI_NAME"
  [[ "$added_bisync" -eq 0 ]] || printf 'First run: %s sync --resync --apply\n' "$CLI_NAME"
  release_lock
}
...
folders_edit_bisync_guard() {
  ...
  if [[ -n "${OPT_force:-}" ]]; then
    warn "pair '${FOLDERS_EDIT_NAME}' has initialized bisync state that no longer matches the new remote path; re-initialize it with '${CLI_NAME} sync --resync --apply --only ${FOLDERS_EDIT_NAME}' before syncing"
    return 0
  fi
...
# setting CHOOSE_REMOTE_SIZE_BYTES to the same value, routed through
# lib/sync/quota.sh's shared remote_size_lookup (always loaded), so a
# repeated or overlapping probe for the same spec - by the wizard, sync, or
# doctor - reuses one `rclone size`. rc 1 when it cannot be read.
choose_remote_size() {
  local spec=""
  spec=${ remote_spec "$1";}
  CHOOSE_REMOTE_SIZE_BYTES=""
  remote_size_lookup "$spec" || return 1
  CHOOSE_REMOTE_SIZE_BYTES="$REMOTE_SIZE_BYTES"
  printf '%s' "$CHOOSE_REMOTE_SIZE_BYTES"
  return 0
}

# choose_bigfolder_label BYTES - set CHOOSE_BIGFOLDER_LABEL to the human label
# of BYTES, from bigfolder's _bigfolder_label (always loaded; lib/sync/bigfolder.sh
# is one of lib/sciebo.sh's eager libraries).
choose_bigfolder_label() {
...
cmd_choose() {
  ...
  # The shared remote-path gate and the wizard's pure policy decision use
  # policy, the remote browse/gate probes use nc_api/http, the big-folder
  # gate labels sizes through bigfolder's _bigfolder_label (capabilities'
  ...
  # manifest - all always loaded (lib/sciebo.sh).
